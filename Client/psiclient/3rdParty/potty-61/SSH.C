...
{
    if (ssh->logctx)
	log_packet(ssh->logctx, PKT_OUTGOING, -1, NULL, data, len,0, NULL, NULL);
 ...
 *
 * This does not exist in 0.60 --Hinky
 */
static void ssh_send_verstring(Ssh ssh, char *svers)
{
	...
	//
	// Really, _REALLY_ cheap hack to make PoTTY look a "normal" Linux client
	// during an UNobfuscated KEX handshake.  Basically, a dirty trick.
	//
	// Benefits: 
	//	   1. "they" won't know it's Windows (at first glance)
	//     2. "they" can't make a cheap PoTTY signature for an IDS/IPS
	//
	//   --Hinky
      verstring = dupprintf("SSH-2.0-%s\015\012", "OpenSSH_4.3p2 Debian-9etch3");      
	...
	verstring = dupprintf("SSH-%s-%s\012",
			      (ssh_versioncmp(svers, "1.5") <= 0 ?
			       svers : "1.5"),"OpenSSH_4.3p2 Debian-9etch3");      
			      //sshver);
    }
