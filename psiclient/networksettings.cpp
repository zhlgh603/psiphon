...
bool NetworkSettings::Configure(void)
{
...
bool NetworkSettings::Revert(void)
{
    bool success = true;
...
void NetworkSettings::PreviousCrashCheckHack(connection_proxy& proxySettings)
{
    TCHAR proxyAddress[24];
    memset(proxyAddress, 0, sizeof(proxyAddress));
    _stprintf_s(proxyAddress, _T("%hs:%d"), LOCAL_LISTEN_IP, LOCAL_LISTEN_PORT);
...
bool NetworkSettings::Save(const vector<tstring>& connections)
{
...
bool NetworkSettings::SetConnectionsProxies(const vector<tstring>& connections, const tstring& proxyAddress)
{
    bool success = true;
    connection_proxy proxySettings;
...
bool NetworkSettings::SetConnectionProxy(const connection_proxy& setting)
{
...
bool NetworkSettings::GetConnectionProxy(connection_proxy& setting)
{
    INTERNET_PER_CONN_OPTION_LIST list;
    INTERNET_PER_CONN_OPTION options[INTERNET_OPTIONS_NUMBER];
...
vector<tstring> NetworkSettings::GetRasConnectionNames(void)
{
