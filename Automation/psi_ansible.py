...
try:
    automation_dir = os.path.join(os.path.abspath('..'), 'Automation')
...
import psi_ops
import psi_ansible_config

from mako.template import Template
from mako.lookup import TemplateLookup
from mako import exceptions

PSI_OPS_DB_FILENAME = os.path.join(automation_dir, 'psi_ops.dat')
MAKO_TEMPLATE = 'psi_ansible_mail_template.mako'
...
def run_playbook(playbook_file=None, inventory=ansible.inventory.Inventory([]),
                 verbose=psi_ansible_config.ANSIBLE_VERBOSE_LEVEL, email_stats=True):
    '''
    ...
    try:
        ...
        if email_stats is True:
            ...
            if 'update' in playbook_file:
                host_output = process_playbook_apt_update_cache(host_output, setup_cache)

            host_output = massage_responses(host_output)
            pkg_result = get_package_result(host_output)
            record = (str(start_time), str(end_time), playbook_file, stats.processed, stats.dark, stats.failures, stats.changed, stats.skipped, res, host_output, pkg_result, setup_cache)
            send_mail(record)
                ...
                if keyword in cache[host]:
                    for k in cache[host][keyword].keys():
                        ...
                        if k == 'stderr' and len(cache[host][keyword][k]) > 0:
                            if 'Reading changelogs' in cache[host][keyword][k]:
                                pass
                            else:
...
def process_playbook_apt_update_cache(host_output, setup_cache):
    ...
    for host_id in host_output:
        ...
        if not 'apt-get update' in host_output[host_id][register_var]['invocation']['module_args']:
            continue
        ...
        for line in host_output[host_id][register_var]['stdout_lines']:
            ...
            if any(s in line for s in ('Preparing to', 'Unpacking', 'Setting up')):
                lines_of_interest.append(line)

            if 'already the newest version' in line:
                lines_of_interest.append(line)

            if any(s in line for s in ('Hit', 'Get', 'Reading package lists')):
                continue

            if 'upgraded' and 'newly installed' and 'remove' in line:  # TODO: Regex probably
    ...
    for host in host_output:
        ...
        for response_num in host_response_count:
            ...
            if host_output[host][response_key]:
                ...
                stderr = host_output[host][response_key]['stderr'] if 'stderr' in host_output[host][response_key] else ''
...
# Getting the newest package information
def get_package_result(massaged_host_output):
    package_output = dict()
    hosts_output = massaged_host_output
    for host in hosts_output:
        for i in range(len(hosts_output[host])):
            if hosts_output[host][i]['module_name'] == 'command':
                if 'dpkg -l' in hosts_output[host][i]['module_details']:
                    package_output[host] = dict()
                if hosts_output[host][i]['stdout_lines']:
                    package_name = hosts_output[host][i]['module_details'].split('-l ', 1)[1]
                    package_version = hosts_output[host][i]['stdout_lines'][-1].split(package_name, 1)[1]
                    package_version = package_version.strip().split(' ', 1)[0]
                    package_output[host][package_name] = package_version

    if len(package_output) > 0:
        version_num = dict()
        for host_id_name in package_output:
            for pkg_name in package_output[host_id_name]:
                pkg_version = package_output[host_id_name][pkg_name]
                if pkg_name not in version_num:
                    version_num[pkg_name] = dict()
                if pkg_version not in version_num[pkg_name]:
                    version_num[pkg_name][pkg_version] = list()
                version_num[pkg_name][pkg_version].append(host_id_name)
...
def main(infile=None, send_mail_stats=False):
    try:
        ...
        if psi_ansible_config.INCLUDE_TEST_GROUP is True:
            print "Creating Test Group"
            test_hosts_list = list()
            for h in psinet_hosts_list:
                if h.id in psi_ansible_config.ANSIBLE_TEST_HOSTS:
                    test_hosts_list.append(h)

            ansible_hosts_list = populate_ansible_hosts(test_hosts_list)
            group = ansible.inventory.Group(psi_ansible_config.ANSIBLE_TEST_GROUP)
            add_hosts_to_group(ansible_hosts_list, group)
            inv.add_group(group)

        # Run against subset
        if psi_ansible_config.INCLUDE_SUBSET_GROUP is True:
            print 'Running Playbook against subset'
            subset_hosts_list = list(psi_ansible_config.ANSIBLE_TEST_DIGITALOCEANS +
                                     psi_ansible_config.ANSIBLE_TEST_LINODES +
                                     psi_ansible_config.ANSIBLE_TEST_FASTHOSTS)
            psinet_hosts_list = [h for h in psinet_hosts_list if h.id in subset_hosts_list]

        if psi_ansible_config.INCLUDE_TEST_GROUP is not True:
        ...
        if psi_ansible_config.ANSIBLE_INCLUDE_BASE_IMAGE is True:
...
if __name__ == "__main__":
    ...
    if options.test_servers:
        psi_ansible_config.INCLUDE_TEST_GROUP = True
    if options.subset:
        psi_ansible_config.INCLUDE_SUBSET_GROUP = True
    if options.base_image:
        psi_ansible_config.ANSIBLE_INCLUDE_BASE_IMAGE = True
